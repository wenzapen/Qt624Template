...
public:
    ...
    YT20_THREAD_SAFE virtual VideoFrameRef getPicture() {NOT_IMPLEMENT_YET}

    YT20_THREAD_SAFE virtual qreal frontPicture() {NOT_IMPLEMENT_YET}

    virtual int skipPicture(const std::function<bool(qreal)> &function) {NOT_IMPLEMENT_YET}

    YT20_THREAD_SAFE virtual AudioFrame getSample() {NOT_IMPLEMENT_YET}

    YT20_THREAD_SAFE virtual qreal frontSample() {NOT_IMPLEMENT_YET}
    ...
    virtual YT20AudioFormat getAudioInputFormat() = 0;

    virtual void setAudioOutputFormat(YT20AudioFormat format) = 0;
...
public:
    explicit DecodeDispatcher(
        ...
        if (description.m_audioStreamsIndex.empty()) {
            result = YT20Player::OpenFileResultType::FAILED;
        ...
        if (isAudio) {
            ...
            result = YT20Player::OpenFileResultType::AUDIO;
        } else {
            result = YT20Player::OpenFileResultType::VIDEO;
    ...
    YT20_THREAD_SAFE VideoFrameRef getPicture() override { return videoDecoder->getPicture(); }

    YT20_THREAD_SAFE qreal frontPicture() override { return videoDecoder->viewFront(); }

    YT20_THREAD_SAFE int skipPicture(const std::function<bool(qreal)> &predicate) override {
        return videoDecoder->skip(predicate);
    }

    YT20_THREAD_SAFE AudioFrame getSample() override { return m_audioDecoder->getSample(); }

    YT20_THREAD_SAFE qreal frontSample() override { return m_audioDecoder->viewFront(); }

    YT20_THREAD_SAFE int skipSample(const std::function<bool(qreal)> &predicate) override {
    ...
    YT20AudioFormat getAudioInputFormat() override {
        return m_audioDecoder->getInputFormat();
    }

    void setAudioOutputFormat(YT20AudioFormat format) override {
...
public:
     ...
     * 保证阻塞获取结果的线程尽快被唤醒, 同时请求 DecodeThread 的工作尽快停止.
     */
    YT20_THREAD_SAFE void statePause() override {
        interrupt = true;
        videoQueue->close();
        qDebug() << "Queue close.";
    }

    YT20_THREAD_SAFE void flush() override {
     ...
     * 保证可以阻塞地获取 Picture 和 Sample. 这个方法是线程安全的.
     */
    YT20_THREAD_SAFE void stateResume() override {
        interrupt = false;
        videoQueue->open();
        emit signalStartWorker(QPrivateSignal());
        qDebug() << "Queue stateResume.";
    ...
    YT20_THREAD_SAFE qreal frontSample() override {NOT_IMPLEMENT_YET}

    YT20_GUARD_BY(DECODER)

        void setEnableAudio(bool enable) override { m_audioDecoder->setEnable(enable); }

    YT20AudioFormat getAudioInputFormat() override { // TODO: IMPLEMENT LATER
        return m_audioDecoder->getInputFormat();
    }

    void setAudioOutputFormat(YT20AudioFormat format) override { // TODO: IMPLEMENT LATER
        m_audioDecoder->setOutputFormat(format);
    }
