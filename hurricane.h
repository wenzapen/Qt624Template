...
public slots:
    ...
    Q_INVOKABLE void openFile(const QString &url, bool autoClose = true) {
        qDebug() << "Hurricane received : Open file:" << url << "state: "<< state;
        ...
        if (state == HurricaneState::CLOSING || state == HurricaneState::INVALID) {
            ...
            // auto whatisit = QUrl::fromLocalFile(url);
            // qDebug() << "hurricane: whatisit: "<< whatisit;
            // emit signalOpenFile(QUrl(url).toLocalFile(), QPrivateSignal());
            emit signalOpenFile(url, QPrivateSignal());
...
private slots:
    ...
    void slotOpenFileResult(YT20Player::OpenFileResultType result) {
