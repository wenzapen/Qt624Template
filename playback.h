...
private:
    ...
    YT20AudioSink *m_audioSink = nullptr;
...
public:
    Playback(Demuxer *demuxer, QObject *parent) : QObject(nullptr), m_demuxer(demuxer) {
        ...
        m_affinityThread->setObjectName(YT20Player::PLAYBACK);
        ...
        connect(this, &Playback::setAudioSpeed, this, [this](qreal speed) {
            ...
            if (speed > YT20AudioSink::MAX_SPEED_FACTOR) {
            ...
            } else if (speed <= YT20AudioSink::MAX_SPEED_FACTOR) {
                if (!this->m_audioSink->isBlock()) { return; }
                // 需要重新启动音频
        ...
        connect(m_affinityThread, &QThread::started, [this] {
            // 在 Playback 线程上初始化
            this->m_audioSink = new YT20AudioSink(YT20Player::DEFAULT_AUDIO_FORMAT);
            connect(m_audioSink, &YT20AudioSink::signalDeviceSwitched, this, [this] {
            ...
            connect(this, &Playback::signalSetSelectedAudioOutputDevice, m_audioSink,
                    &YT20AudioSink::requestDeviceSwitch);
            connect(m_audioSink, &YT20AudioSink::signalAudioOutputDeviceListChanged, this, [this] {
    ...
    void setDesiredFormat(const YT20AudioFormat& format) {
