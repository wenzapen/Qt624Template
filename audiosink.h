 ...
 * 中的音频. 当 AudioBuffer 数据不足时, 系统会通过回调函数从 DataBuffer 中获取数据. DataBuffer 由 YT20AudioSink
 * 维护, 当需要播放音频时需要先调用 write 函数将音频数据写入 DataBuffer.
 */
class YT20AudioSink : public QObject {
    Q_OBJECT
private:
    ...
    YT20AudioFormat m_format;
    YT20AudioFormat m_deviceFormat;
    ...
    void initializeStream() {
        if (!paInitialized) {
            Pa_Initialize();
            paInitialized = true;
            qDebug() << "Initialize YT20AudioSink backend.";
        }
        ...
        ASSERT_PA_OK(
            Pa_OpenStream(&m_stream, nullptr, param, m_format.getSampleRate(), paFramesPerBufferUnspecified,
                          ...
                          [](
                              ...
                              return static_cast<YT20AudioSink *>(userData)->m_paCallback(inputBuffer, outputBuffer,
                                                                                          framesPerBuffer,
                                                                                          timeInfo);
                          }, this),
        ...
        m_deviceFormat = YT20AudioFormat(YT20Player::Int16, static_cast<int>(info->sampleRate),
                                         param->channelCount);
        ASSERT_PA_OK(Pa_SetStreamFinishedCallback(m_stream, [](void *userData) {
                         static_cast<YT20AudioSink *>(userData)->m_paStreamFinishedCallback();
...
public:
    ...
    /**
     * 创建YT20AudioSink并attach到默认设备上
     * @param format 音频格式
     * @param bufferSizeAdvice DataBuffer 的建议大小, YT20AudioSink 保证实际的 DataBuffer 不小于建议大小.
     */
    explicit YT20AudioSink(YT20AudioFormat format) : m_volume(0.5), m_pitch(1.0), m_state(PlaybackState::STOPPED),
        m_format(std::move(format)),
        m_deviceFormat(YT20Player::Unknown, 0, 0), m_speedFactor(1.0) {
        ...
        connect(hotPlugDetector, &HotPlugDetector::audioOutputsChanged, this,
                &YT20AudioSink::onAudioOutputDevicesChanged);
    }

    /**
     * 析构即从deattach当前设备
     */
    ~YT20AudioSink() override {
        std::lock_guard lock(paStreamLock);
        m_state = PlaybackState::STOPPED;
        PaError err = Pa_CloseStream(m_stream);
        m_stream = nullptr;
        if (err != paNoError) {
            qWarning() << "Error at Destroying YT20AudioSink" << Pa_GetErrorText(err);
        }
    }

    /**
     * 开始播放, 状态变为 PlaybackState::PLAYING. 若当前DataBuffer内容不足, 状态将会发生改变.
     * @see YT20AudioSink::stateChanged
     * @see YT20AudioSink::resourceInsufficient
     */
    void start() {
        std::lock_guard lock(paStreamLock);
    ...
    bool write(const char *buf, qint32 origLen) {
        int len = 0;
        if (m_format.getSampleFormat() != YT20Player::Int16) {
    ...
    void setFormat(const YT20AudioFormat &format) {
        qDebug() << "audiosink: setFormat";
        std::unique_lock lock(paStreamLock);
        // m_format = {YT20Player::Int16, format.getSampleRate(), format.getChannelCount()};
        // m_format = YT20Player::DEFAULT_AUDIO_FORMAT;
        qDebug() << "audiosink: setFormat: crash here?";
        lock.unlock();
...
public slots:
    ...
    YT20AudioFormat getCurrentDeviceFormat() {
