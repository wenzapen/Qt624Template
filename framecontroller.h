...
public:
    explicit FrameController([[maybe_unused]] QObject *parent) : QObject(nullptr) {
        ...
        m_affinityThread->setObjectName(YT20Player::FRAME);
...
private slots:
    ...
    void initOnThread() {
        ...
        connect(m_demuxer, &Demuxer::openFileResult, this, [this](YT20Player::OpenFileResultType result) {
            if (result != YT20Player::OpenFileResultType::FAILED) {
...
public:
    ...
    YT20_THREAD_SAFE void backward() {
        emit signalBackward();
    }

    YT20_THREAD_SAFE void forward() {
        emit signalForward();
    }
...
public slots:
    ...
    void openFile(const QString &path) {
        qDebug() << "frame contoller:  Open file" << path;
...
signals:
    ...
    void openFileResult(YT20Player::OpenFileResultType result);
